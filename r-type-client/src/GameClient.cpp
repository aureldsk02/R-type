...
GameClient::GameClient()
    : displayHandler(), inputHandler(), audioHandler(), networkClient(nullptr), 
      sceneHandler(), sequenceNumber(1), isRunning(false), connectPacketSent(false), gameStarted(false), 
      gameOver(false), pendingDisconnect(false), multiplayerMode(false), inMenu(true), inGameModeMenu(false), inMultiplayerForm(false), inSettingsMenu(false), inHelpMenu(false), inRoomMenu(false), inRoomListMenu(false), inRoomLobby(false),
      playerName(""), serverIP(""), serverPort(0), playerData{}, playerMovePacket{}, previousPosition{0, 0}, 
      currentMove(STOP), statusTextIndex(0), playerSpriteIndex(0), playerAnimationIndex(0), player{} {
    ...
    if (action == "joinRoom") {
        ...
        if (roomId != 0 && networkClient) {
            if (playerData.room_id == roomId) {
                ...
                roomLobby.SetRoomInfo(std::string("Room ") + std::to_string(roomId), static_cast<int>(roomId), 4);
...
void GameClient::Initialize(const std::string& name)
{
    ...
    //     std::cout << "Added test player sprite at position (400, 400)" << std::endl;
    // }
    ...
    //     std::cout << "Added test enemy sprite at position (600, 200)" << std::endl;
    //     
    //     if (enemies.size() > 1) {
    ...
    //         std::cout << "Added test enemy2 sprite at position (800, 300)" << std::endl;
    //     }
    // }
    // 
    ...
    //     std::cout << "Added test fire sprite at position (500, 350)" << std::endl;
    // }
}
...
void GameClient::createPlayerEntity()
{
    ...
    if (!players.empty()) {
        ...
        sceneHandler.getcurrentGame().addEntity("Player", fallbackPlayer, player.id);
    }
}
...
void GameClient::handlePlayerJoin(const PlayerJoinPacket& packet)
{
    ...
    std::cout << "Player Name: " << std::string(reinterpret_cast<const char*>(packet.player_name)) << std::endl;
    ...
    playerPreviousPositions[playerData.player_id] = {static_cast<int32_t>(playerData.x), static_cast<int32_t>(playerData.y)};
    std::cout << "[INIT] Position initiale du joueur " << playerData.player_id << ": (" << playerData.x << ", " << playerData.y << ")" << std::endl;

    playerMovePacket.player_id = playerData.player_id;
    playerMovePacket.room_id = playerData.room_id;
    if (pendingLobbyJoin && (pendingLobbyRoomId == 0 || pendingLobbyRoomId == playerData.room_id)) {
        inRoomMenu = false;
        inRoomListMenu = false;
        if (!inRoomLobby) {
            roomLobby.Initialize();
    ...
    if (inRoomLobby) {
        roomLobby.SetRoomInfo(std::string("Room ") + std::to_string(playerData.room_id), static_cast<int>(playerData.room_id), 4);
        roomLobby.SetIsRoomOwner(false);
        if (networkClient) {
    ...
    try {
        std::string joinedName = std::string(reinterpret_cast<const char*>(packet.player_name));
        ...
        if (packet.player_id != player.id) {
            playerPreviousPositions[packet.player_id] = {static_cast<int32_t>(packet.x), static_cast<int32_t>(packet.y)};
            std::cout << "[INIT] Position initiale du joueur distant " << packet.player_id << ": (" << packet.x << ", " << packet.y << ")" << std::endl;
        }
    } catch (...) {}

    displayHandler.UpdateText(statusTextIndex, "Connected! Player ID: " + std::to_string(playerData.player_id) + ", Room: " + std::to_string(playerData.room_id));
}
...
void GameClient::handlePlayerDisconnect(const PlayerDisconnectPacket& packet)
{
    ...
    std::cout << "  - gameStarted: " << (gameStarted ? "true" : "false") << std::endl;
    std::cout << "  - inRoomLobby: " << (inRoomLobby ? "true" : "false") << std::endl;
    std::cout << "  - multiplayerMode: " << (multiplayerMode ? "true" : "false") << std::endl;
    std::cout << "========================================" << std::endl;
    
    auto allEntities = sceneHandler.getcurrentGame().getallEntity();
    auto found = std::find_if(allEntities.begin(), allEntities.end(), 
        [&packet](const auto& p) { return p.second == static_cast<int>(packet.player_id); });
    
    if (found != allEntities.end()) {
        std::cout << "[ACTION] Suppression de l'entité visuelle du joueur " << packet.player_id << std::endl;
        sceneHandler.getcurrentGame().removeEntity(packet.player_id);
    } else {
        std::cout << "[INFO] Aucune entité visuelle trouvée pour le joueur " << packet.player_id << std::endl;
    }
    
    auto posIt = playerPreviousPositions.find(packet.player_id);
    if (posIt != playerPreviousPositions.end()) {
        std::cout << "[ACTION] Suppression de la position stockée du joueur " << packet.player_id 
                  << " (était à: " << posIt->second.x << ", " << posIt->second.y << ")" << std::endl;
        playerPreviousPositions.erase(packet.player_id);
    } else {
        std::cout << "[INFO] Aucune position stockée pour le joueur " << packet.player_id << std::endl;
    }
    
    if (inRoomLobby) {
        std::cout << "[ACTION] Retrait du joueur " << packet.player_id << " du lobby" << std::endl;
    ...
    if (gameStarted && !inRoomLobby) {
        ...
        std::cout << "Raison: Déconnexion du joueur " << packet.player_id << std::endl;
        std::cout << "========================================" << std::endl;
        ...
        std::cout << "[INFO] Déconnexion dans le lobby - simple retrait du joueur" << std::endl;
    }
...
void GameClient::handleGameStart(const GameStartPacket& packet)
{
    ...
    if (audioHandler.LoadMusic("res/music/Naruto Akatsuki Theme Song.mp3", "Akatsuki Theme")) {
        audioHandler.PlayMusic();
        std::cout << "Musique Naruto Akatsuki lancée - La partie commence!" << std::endl;
    } else {
        std::cout << "Impossible de charger la musique Naruto Akatsuki" << std::endl;
    }
}
...
void GameClient::handleGameAi(const GameAiPacket& packet)
{
    try {
        ...
        auto allEntities = sceneHandler.getcurrentGame().getallEntity();
        auto entity = find_if(allEntities.begin(), allEntities.end(), [id](const auto& p) { return p.second == id; });
        
        std::string subtype;
        switch (packet.entity_type) {
            case 0: subtype = "BASIC_ENEMY"; break;
            case 1: subtype = "FIRING_ENEMY"; break;
            case 2: subtype = "FOLLOWING_ENEMY"; break;
            default: subtype = "BASIC_ENEMY"; break;
        }
        ...
        if (entity != allEntities.end()) {
            // std::cout << "Updating entity ID: " << id << " to position (" << packet.position_x << ", " << packet.position_y << ")" << std::endl;
            sceneHandler.getcurrentGame().Update(GetFrameTime(), id, Vector2{packet.position_x, packet.position_y});
        } else {
...
{
    std::cout << "damage collision between " << packet.entity_id_a << " && " << packet.entity_id_b << std::endl;
    if (packet.player_id == player.id) {
...
void GameClient::handlePlayerHasMove(const PlayerHasMovePacket& packet)
{
    ...
    std::cout << "Player " << packet.player_id << " position: (" << server_x << ", " << server_y << ")" << std::endl;

    if (packet.player_id == player.id) {
        validateServerMove(packet, server_x, server_y);
    } else {
        auto allEntities = sceneHandler.getcurrentGame().getallEntity();
        auto found = find_if(allEntities.begin(), allEntities.end(), [&packet](const auto& p) { return p.second == (int)packet.player_id; });
        if (found == allEntities.end()) {
            auto players = displayHandler.GetSpritesPlayer();
            ...
            sceneHandler.getcurrentGame().addEntity("Player", remote, (int)packet.player_id);
        } else {
            sceneHandler.getcurrentGame().Update(GetFrameTime(), (int)packet.player_id, Vector2{server_x, server_y});
        }
    }
}
...
void GameClient::handleBullet(const BulletPacket& packet)
{
    ...
    //           << ", Direction: (" << packet.direction_x << ", " << packet.direction_y << ")"
    ...
    if (entity != allEntities.end()) {
        sceneHandler.getcurrentGame().Update(GetFrameTime(), id, Vector2{packet.position_x, packet.position_y});
        // std::cout << "Updated bullet ID " << id << " position to (" << packet.position_x << ", " << packet.position_y << ")" << std::endl;
    } else {
        SpriteComponent bulletSprite;
        
        auto fire = displayHandler.GetSpritesFire();
        if (!fire.empty()) {
            if (packet.is_player_bullet) {
                bulletSprite = fire[1];
            } else {
                bulletSprite = fire[0];
            }
            std::cout <<"display rect: " << bulletSprite.sourceRect.x << ", " << bulletSprite.sourceRect.y << ", "
                      << bulletSprite.sourceRect.width << ", " << bulletSprite.sourceRect.height << std::endl;
            bulletSprite.scale = 0.8f;
            std::cout << "Created bullet ID " << id << " using fire sprite" << std::endl;
        } else {
            bulletSprite.visible = true;
            bulletSprite.scale = 0.5f;
            bulletSprite.tint = {255, 255, 0, 255};
            std::cout << "Created bullet ID " << id << " using fallback sprite" << std::endl;
        }
...
void GameClient::handleMultiplayerFormAction(const std::string& action)
{
    ...
    if (action == "connect") {
        std::string newPlayerName = sceneHandler.getCurrentMultiplayerForm().GetPlayerName();
        std::string newServerIP = sceneHandler.getCurrentMultiplayerForm().GetServerIP();
        int newServerPort = sceneHandler.getCurrentMultiplayerForm().GetServerPort();
        
        std::cout << "Player " << newPlayerName << " connecting to server: " << newServerIP << ":" << newServerPort << std::endl;
...
void GameClient::connectToServer(const std::string& ip, int port)
{
    ...
    sceneHandler.getCurrentMultiplayerForm().SetConnectionStatus(STATUS_CONNECTING, "");
    ...
    try {
        ...
        networkClient->set_player_connect_callback([this](const PlayerConnectPacket& packet) {
            this->handlePlayerConnect(packet);
        });
        networkClient->set_player_join_callback([this](const PlayerJoinPacket& packet) {
            this->handlePlayerJoin(packet);
        });
        networkClient->set_player_disconnect_callback([this](const PlayerDisconnectPacket& packet) {
            this->handlePlayerDisconnect(packet);
        });
        networkClient->set_game_start_callback([this](const GameStartPacket& packet) {
            this->handleGameStart(packet);
        });
        networkClient->set_game_ai_callback([this](const GameAiPacket& packet) {
            this->handleGameAi(packet);
        });
        networkClient->set_game_collision_callback([this](const GameCollisionPacket& packet) {
            this->handleGameCollision(packet);
        });
        networkClient->set_player_has_move_callback([this](const PlayerHasMovePacket& packet) {
            this->handlePlayerHasMove(packet);
        });
        networkClient->set_bullet_callback([this](const BulletPacket& packet) {
            this->handleBullet(packet);
        });
        networkClient->set_room_list_callback([this](const RoomListPacket& packet) {
            this->handleRoomList(packet);
        });
        networkClient->set_room_players_callback([this](const Sendplayer_in_room_Packet& packet) {
            try {
                if (!inRoomLobby) return;
                if (packet.room_id != playerData.room_id) {
                    if (!(pendingLobbyJoin && pendingLobbyRoomId == packet.room_id)) return;
                }
                bool localReady = roomLobby.IsLocalPlayerReady();
                roomLobby.Cleanup();
                const char* base = reinterpret_cast<const char*>(packet.player_names);
                ...
                roomLobby.SetPlayerReady(static_cast<int>(playerData.player_id), localReady);
            } catch (...) {}
        });
        
        networkClient->set_chat_broadcast_callback([this](const MessagePacket& packet) {
            std::string message(reinterpret_cast<const char*>(packet.message));
            std::cout << "[CHAT] Message reçu du joueur " << packet.player_id << ": " << message << std::endl;
            ...
            Color messageColor = (packet.player_id == player.id) ? Color{100, 255, 100, 255} : Color{200, 200, 255, 255};
...
void GameClient::handleRoomAction(const std::string& action)
{
    ...
    if (action == "createRoom") {
        std::cout << "Create Room pressed: sending PlayerConnectPacket + RoomListRequestPacket" << std::endl;
        if (networkClient) {
            networkClient->send_connect_packet(playerName);
            networkClient->send_get_room_list();
        }
        inRoomMenu = false;
        inRoomListMenu = true;
        try { sceneHandler.getCurrentRoomListMenu().ClearRooms(); } catch (...) {}
        
    } else if (action == "joinRoom") {
        std::cout << "Join Room pressed: sending PlayerConnectPacket and opening lobby view (server assigns room)" << std::endl;
        if (networkClient) {
            networkClient->send_connect_packet(playerName);
        }
        inRoomMenu = false;
...
void GameClient::Update()
{
    ...
    updateCount++;
    if (updateCount <= 5) {
        std::cout << "Update #" << updateCount << " - inMenu: " << inMenu << std::endl;
    }
    
    if (pendingDisconnect) {
        std::cout << "[PENDING DISCONNECT] Traitement de la déconnexion différée..." << std::endl;
        ...
        if (networkClient) {
            std::cout << "[DISCONNECT] Envoi du paquet de déconnexion..." << std::endl;
    ...
    if (multiplayerMode && networkClient && inputHandler.IsKeyPressed(InputKey::KEY_SPACE)) {
        std::cout << "SPACE pressed: sending CreateBulletPacket (playerId=" << player.id
                  << ", room=" << playerData.room_id << ")" << std::endl;
    ...
    if (inMenu) {
        ...
        if (sceneHandler.getCurrentGameModeMenu().IsActionSelected()) {
            handleGameModeAction(sceneHandler.getCurrentGameModeMenu().GetSelectedAction());
        ...
        if (sceneHandler.getCurrentMultiplayerForm().IsActionSelected()) {
            handleMultiplayerFormAction(sceneHandler.getCurrentMultiplayerForm().GetSelectedAction());
        ...
        if (sceneHandler.getCurrentSettingsMenu().IsActionSelected()) {
            handleSettingsAction(sceneHandler.getCurrentSettingsMenu().GetSelectedAction());
        ...
        if (sceneHandler.getCurrentRoomListMenu().IsActionSelected()) {
            handleRoomListAction(sceneHandler.getCurrentRoomListMenu().GetSelectedAction());
        ...
        if (lobbyPlayersRefreshTimer >= 1.0f && networkClient) {
            uint32_t rid = (pendingLobbyJoin && pendingLobbyRoomId != 0) ? pendingLobbyRoomId : playerData.room_id;
            if (rid != 0) {
        ...
        if (inputHandler.IsKeyPressed(InputKey::KEY_ESCAPE)) {
            if (gameOver) {
                ...
                try {
                    ...
                    if (audioHandler.LoadSound()) {
                        audioHandler.PlaySound();
                        std::cout << "[AUDIO] Son de retour au menu joué (hover)" << std::endl;
                    }
                } catch (...) {
                    std::cout << "[AUDIO] Son de retour au menu non disponible" << std::endl;
                }
        ...
        if (inputHandler.IsKeyHeld(InputKey::KEY_W) || inputHandler.IsKeyHeld(InputKey::KEY_UP)) {
            newMove = UP;
        } else if (inputHandler.IsKeyHeld(InputKey::KEY_S) || inputHandler.IsKeyHeld(InputKey::KEY_DOWN)) {
            newMove = DOWN;
        } else if (inputHandler.IsKeyHeld(InputKey::KEY_A) || inputHandler.IsKeyHeld(InputKey::KEY_LEFT)) {
            newMove = LEFT;
        } else if (inputHandler.IsKeyHeld(InputKey::KEY_D) || inputHandler.IsKeyHeld(InputKey::KEY_RIGHT)) {
            newMove = RIGHT;
        }
        ...
        int c = 0;
            ...
            for (auto _entity :  registry.getEntitiesWith<Team>()){
                ...
                sceneHandler.getcurrentGame().Update(GetFrameTime(), 0, Vector2{static_cast<float>(pos.x), static_cast<float>(pos.y)});
            }
            ...
            for (Entity e : entities) {
                ...
                if (team.ind == Team::Individu::PLAYER && !registry.hasComponent<Health>(e)) {
                            ...
                            auto entity = find_if(allEntities.begin(), allEntities.end(), [id](const auto& p) { return p.second == id; });
                            if (entity != allEntities.end()) {
                                sceneHandler.getcurrentGame().Update(GetFrameTime(), id, Vector2{pos.x, pos.y});
                                // std::cout << "Updated bullet ID " << id << " position to (" << packet.position_x << ", " << packet.position_y << ")" << std::endl;
                            } else {
                                SpriteComponent bulletSprite;
                                auto fire = displayHandler.GetSpritesFire();
                                if (!fire.empty()) {
                                    if (_bullet_packet.is_player_bullet) {
                                        bulletSprite = fire[1];
                                    } else {
                                        bulletSprite = fire[0];
                                    }
...
void GameClient::DisplayGame()
{
    ...
    if (inMenu) {
        ...
        if (GetTime() - lastPrint > 3.0f) {
            ...
            std::cout << "Background: " << bg.textureName << " visible=" << bg.visible << std::endl;
        ...
        TextComponent scoreDisplay = fontManager.CreateText(scoreText, 
                                                          {scoreX, scoreY}, 
                                                          FontManager::PIXEL_REGULAR, 
                                                          FontManager::HUGE_SIZE,
        ...
        if (multiplayerMode) {
            ...
            if (!gameChat.IsVisible()) {
                ...
                TextComponent chatIndicator = fontManager.CreateText("Appuyez sur [T] pour ouvrir le chat", 
        ...
        if (gameOver) {
            ...
            TextComponent gameOverText = fontManager.CreateText(gameOverStr, 
                                                              {static_cast<float>((screenWidth - gameOverWidth) / 2), static_cast<float>(screenHeight / 2 - 80)}, 
                                                              FontManager::PIXEL_REGULAR, 
                                                              FontManager::HUGE_SIZE, 
                                                              Color{255, 0, 0, 255});
            ...
            int disconnectWidth = MeasureText(disconnectStr.c_str(), disconnectFontSize);
            TextComponent disconnectMsg = fontManager.CreateText(disconnectStr, 
                                                                {static_cast<float>((screenWidth - disconnectWidth) / 2), static_cast<float>(screenHeight / 2)}, 
                                                                FontManager::PIXEL_REGULAR, 
                                                                FontManager::MEDIUM, 
                                                                Color{255, 255, 255, 255});
            ...
            int instructionWidth = MeasureText(instructionStr.c_str(), instructionFontSize);
            TextComponent instructionText = fontManager.CreateText(instructionStr, 
                                                                  {static_cast<float>((screenWidth - instructionWidth) / 2), static_cast<float>(screenHeight / 2 + 60)}, 
                                                                  FontManager::PIXEL_REGULAR, 
                                                                  FontManager::SMALL, 
...
{
    std::cout << "Executing move " << move << " with sequence " << sequence << std::endl;
...
void GameClient::validateServerMove(const PlayerHasMovePacket& packet, float server_x, float server_y)
{
    // std::cout << "Validating server move with sequence " << packet.header.seq << std::endl;
    ...
    if (it != pendingMoves.end()) {
        ...
        if (diff_x > tolerance || diff_y > tolerance) {
            ...
            std::cout << "Client: (" << current_x << ", " << current_y << ")" << std::endl;
            std::cout << "Server: (" << server_x << ", " << server_y << ")" << std::endl;
        ...
        if (packet.header.seq > 0) {
            // std::cout << "Warning: Received validation for unknown sequence " << packet.header.seq << std::endl;
        }
    }
}

void GameClient::reconcilePosition(float server_x, float server_y, uint32_t server_sequence)
{
